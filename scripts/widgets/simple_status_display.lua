...
local SimpleStatusDisplay = Class(Widget, function(self, owner)
...
function SimpleStatusDisplay:UpdateValues()
...
function SimpleStatusDisplay:OnUpdate()

	local target = nil

	local ents = TheInput:GetAllEntitiesUnderMouse()
	for k,v in pairs(ents) do
		if v:HasTag("player") and v.Transform then
			target = v
			break
		end
	end
...
function SimpleStatusDisplay:UpdatePosition(x,y)
    self:SetPosition(x+75,y-10,0)
end

function SimpleStatusDisplay:FollowMouseConstrained()
...
function SimpleStatusDisplay:HealthDelta(data)
...
function SimpleStatusDisplay:HungerDelta(data)
